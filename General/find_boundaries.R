...
find_boundaries <- function(data, window_size=10, valley_height_threshold=0.3, na_rm=T){
  ...
  if (is.null(valleys)) return("No boundaries detected. Consider adjusting parameter values.")
  # Return output
  valleys[,1] <- -valleys[,1]
  colnames(valleys) <- c("Signal", "Valley_position", "Valley_start", "Valley_end")
